...
final class LessonController
{
        ...
        try {
            ...
            if ($method === 'GET' && $action === 'show') {
                ...
                foreach ($this->lessonService->attendance($lesson->getId()) as $item) {
                    $attendance[] = ['id' => $item->getId(), 'studentId' => $item->getStudentId(), 'status' => $item->getStatus()->value];
                }

                $this->respond(200, ['data' => $this->data($lesson), 'attendance' => $attendance]);
                return;
            }
            if ($method === 'POST' && $action === 'create') {$lesson = $this->lessonService->create($this->post('lessonDate'), $this->post('plannedContent'), $this->post('content'), $this->id('classId'));
                $this->respond(201, ['data' => $this->data($lesson)]);return;}
            if ($method === 'POST' && $action === 'edit') {$lesson = $this->lessonService->update($this->id('id'), $this->post('lessonDate'), $this->post('plannedContent'), $this->post('content'), $this->id('status'));
                $this->respond(200, ['data' => $this->data($lesson)]);return;}
            if ($method === 'POST' && $action === 'register-attendance') {$this->lessonService->registerAttendance($this->id('id'), $this->id('studentId'), $this->id('status'));
                $this->respond(200, ['data' => ['message' => 'Presença registrada com sucesso.']]);return;}
    ...
    {return ['id' => $lesson->getId(), 'lessonDate' => $lesson->getLessonDate(), 'plannedContent' => $lesson->getPlannedContent(), 'content' => $lesson->getContent(), 'status' => $lesson->getStatus(), 'classId' => $lesson->getClassId()];}
