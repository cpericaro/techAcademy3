    ...
    public function handle(string $method, string $action): void
    {
        try {
            ...
            if ($method === 'POST' && $action === 'create') {
                $class = $this->classService->create($this->post('name'), $this->id('year'));
                $this->respond(201, ['data' => $this->data($class)]);
                return;
            }

            if ($method === 'POST' && $action === 'edit') {
                $class = $this->classService->update($this->id('id'), $this->post('name'), $this->id('year'));
                $this->respond(200, ['data' => $this->data($class)]);
                return;
            }

            if ($method === 'POST' && $action === 'add-student') {
                $this->classService->addStudent($this->id('id'), $this->id('studentId'));
                $this->respond(200, ['data' => ['message' => 'Aluno adicionado à turma.']]);
                return;
            }

            if ($method === 'POST' && $action === 'remove-student') {
                $this->classService->removeStudent($this->id('id'), $this->id('studentId'));
                $this->respond(200, ['data' => ['message' => 'Aluno removido da turma.']]);
                return;
            }

            if ($method === 'POST' && $action === 'add-schedule') {
                $schedule = $this->classService->addSchedule($this->id('id'), $this->id('weekday'), $this->post('startTime'), $this->post('endTime'));
                $this->respond(201, ['data' => ['id' => $schedule->getId()]]);
                return;
            }

            if ($method === 'POST' && $action === 'remove-schedule') {
                $this->classService->removeSchedule($this->id('id'), $this->id('scheduleId'));
                $this->respond(200, ['data' => ['message' => 'Horário removido com sucesso.']]);
                return;
            }

            if ($method === 'POST' && $action === 'delete') {
                $this->classService->delete($this->id('id'));
                $this->respond(200, ['data' => ['message' => 'Turma excluída com sucesso.']]);
                return;
            }
